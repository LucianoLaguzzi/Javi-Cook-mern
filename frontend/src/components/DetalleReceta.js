...
const DetalleReceta = () => {
    ...
    try {
      const response = await axios.post(
        `https://javicook-mern.onrender.com/api/recetas/${id}/comentarios`,
        {
          ...
          parentCommentId: parentId || comentarioAResponder // Si es respuesta, se asigna el parent
        }
      );
  ...
  const responderComentario = (comentarioId, isRespuesta = false) => {
    setComentarioAResponder(comentarioId); // Establecer el comentario o respuesta a responder
    setEsRespuesta(isRespuesta); // Definir si es una respuesta a una respuesta
  };
                ...
                {comentarios && comentarios.length > 0 ? (
                 comentarios.map((comentario) => (
                  <div key={comentario._id} className="contenedores-spam">
                          ...
                          <div className="respuestas">
                            {comentario.respuestas.map((respuesta) => (
                              <div key={respuesta._id} className="respuesta-comentario">
                                <div className="imagen-nombre">
                                  <img className="imagen-perfil-comentario" src={respuesta.usuario.imagenPerfil || "../images/default-imagen-perfil"} alt={respuesta.usuario.nombre} />
                                ...
                                <button className="boton-responder" onClick={() => responderComentario(respuesta._id, true)}>Responder</button>
                              </div>
                            ))}
                          </div>
                        )}
                    ...
                    {comentario.respuestas.map((respuesta) => (
                      respuesta._id === comentarioAResponder && (
                        <div key={respuesta._id} className="input-respuesta">
                          <input
                            type="text"
                            value={respuesta}
                            onChange={(e) => setRespuesta(e.target.value)}
                            placeholder="Escribe tu respuesta..."
                          />
                          <button onClick={() => agregarRespuesta(respuesta._id)}>Enviar</button>
                        </div>
                      )
                    ))}
                  </div>
